...
4.  **Pip & Thư viện Python:** Cài đặt các gói phụ thuộc:
    ```bash
...
### 3.2. Triển khai Hạ tầng bằng CloudFormation (Lab 2 Part 2)

> **LƯU Ý QUAN TRỌNG VỀ MÔI TRƯỜNG AWS ACADEMY:**
> Tài khoản sandbox áp dụng chính sách SCP chặn quyền tạo các tác vụ AWS CodeBuild và AWS CodePipeline (gây lỗi khi chạy tài nguyên trong [pipeline.tf](./terraform/pipeline.tf)).
> **Giải pháp khắc phục:** Quy trình tự động hóa kiểm định và deploy CloudFormation được chuyển dịch hoàn toàn sang **GitHub Actions** thông qua file cấu hình [.github/workflows/deploy-cfn.yml](./.github/workflows/deploy-cfn.yml). Các tài nguyên CodeBuild/CodePipeline lý thuyết trong `pipeline.tf` được comment lại nhằm mục đích phục vụ công tác chấm điểm cấu trúc hạ tầng.

#### Cách chạy kiểm thử hạ tầng CloudFormation cục bộ:
1.  Kiểm tra lỗi cú pháp và định dạng cấu trúc template:
    ```bash
    cfn-lint cloudformation/main.yaml
    ```
2.  Chạy kiểm thử tích hợp tự động (tạo và xóa stack trên môi trường AWS thực tế):
    ```bash
    taskcat test run -t cloudformation/main.yaml --regions us-east-1
    ```

#### Tự động hóa qua GitHub Actions (`.github/workflows/deploy-cfn.yml`):
*   **Trình kích hoạt:** Tự động kích hoạt khi push mã nguồn vào thư mục `cloudformation/**`.
*   **Tiến trình thực thi:**
    1.  Cài đặt môi trường Python, `cfn-lint` và `taskcat`.
    2.  Thực hiện quét lỗi cú pháp template qua lệnh `cfn-lint`.
        *   *Minh chứng logs chạy cfn-lint & taskcat:*
            ![cfn-lint and taskcat Validation Logs](images/cfn_logs.png)
    3.  Khởi tạo kết nối AWS qua AWS Credentials lưu trong GitHub Secrets.
    4.  Chạy `taskcat` xác thực tính tương thích hạ tầng.
    5.  Sử dụng Action `aws-actions/aws-cloudformation-github-deploy` để deploy/update Stack `Devops-Lab1-Stack` lên vùng mạng `us-east-1` bằng quyền của Role `LabRole` (`arn:aws:iam::568004971730:role/LabRole`).
        *   *Minh chứng Pipeline CloudFormation:*
            ![CloudFormation CI/CD Workflow](images/cfn_pipeline.png)
        *   *Stack CloudFormation được tạo thành công trên AWS:*
            ![CloudFormation Stack Create Complete](images/cfn_stack.png)
